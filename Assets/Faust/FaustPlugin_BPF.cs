...
public class FaustPlugin_BPF: MonoBehaviour {
    ...
    * @param x New parameter value */
    ...
    {
		is_virtual = this.gameObject.GetComponent<AudioSource>().isVirtual;
		is_playing = this.gameObject.GetComponent<AudioSource>().isPlaying;
	}
    ...
    */
	private void OnAudioFilterRead(float[] buffer, int numchannels) {
        if(is_playing&&!is_virtual) ctx.process(buffer, buffer.Length / numchannels, numchannels);
	}
